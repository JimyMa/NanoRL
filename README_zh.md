...
两种 trainer 启动方式都可以保存 HF 格式 checkpoint：

- `nanorl train`：本地 / `torchrun` 直接启动 trainer；训练进程跑在执行命令的机器上。
- `nanorl train-ray`：Ray 管理 trainer；driver 可以在别的节点执行，Ray 会把
  TrainActors 放到 `--train-ip` 指定的训练节点。
